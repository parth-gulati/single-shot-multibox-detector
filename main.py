...
model = SSD(class_num=4).to(device)
...
if not args.test:
    ...
    for i, data in enumerate(dataloader_test, 0):
        ...
        values = generate_mAP(i, pred_box_, pred_confidence_, ann_box_, ann_confidence_, 0.5, 0.5, boxs_default)
        image_data.append(values)
        
        #TOD: save predicted bounding boxes and classes to a txt file.
